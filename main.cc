...
int main() {
  ...
  "last_price,previous_close,open,high_limit,low_limit,total_trade,total_volume,total_value,"<<
  ...
  while (!ifs.eof()) {
    ...
    if (buffer[0] == 0x1b) {
      ...
      << body.total_volume_ << ","
      << body.total_value_ << ","
      << body.average_ask_price_ << ","
      << body.average_bid_price_ << ","
      << body.average_ask_volume_ << ","
      << body.average_bid_volume_ << ","
      << body.ask_price1_ << ","
      << body.ask_volume1_ << ","
      << body.bid_price1_ << ","
      << body.bid_volume1_ << ","
