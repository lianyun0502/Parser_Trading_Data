...
void TWSEDataBody6::ParseBody(const char* data) {
  ...
  if ((status_1 >> 7) & 0b1) {
    ...
    offset += 5;
    if (status_2 &
        0b11) {  // 揭示最近一筆成交價，成交量則以 0 揭示，買賣價量均不揭示
      total_trade_ = 0;
    } else {  // 一般揭示
...
int64_t TWSEDataBody6::FmtTimetoTimePoint(const char* data) {
  ...
  int millisecond =
      BCDToInt16(data + 19, 1) * 10 + BCDToInt16(data + 20, 1) / 10;
  int microsecond =
      (BCDToInt16(data + 20, 1) % 10) * 100 + BCDToInt16(data + 21, 1);
  t += chrono::milliseconds(millisecond);
  t += chrono::microseconds(microsecond);

  return chrono::duration_cast<std::chrono::microseconds>(t.time_since_epoch())
      .count();
}